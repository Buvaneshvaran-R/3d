...
const CATMark = () => {
  ...
  const loadMarks = async () => {
    try {
      ...
      if (studentId) {
        const saved = localStorage.getItem(`cat_marks_${studentId}`);
        if (saved) {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {/* Admin Editor */}
      {isAdmin() && <AdminMarksEditor type="cat" title="Manage CAT Marks" maxMarks={50} />}
              ...
              <table className="data-table">
                ...
                <tbody>
                  {subject.marks.map((mark: { subjectName: string; subjectCode: string; score: number; maxMarks: number; remarks: string; date: string }, index: number) => {
                    ...
                    return (
                      <tr key={index}>
                        <td className="font-medium">Test {mark.testNumber}</td>
      ...
      {marks.length === 0 && (
        <Card className="border-none shadow-card">
          <CardContent className="p-12 text-center">
            ...
            <p className="text-muted-foreground">No CAT marks available yet.</p>
          </CardContent>
        </Card>
      )}
    </div>
