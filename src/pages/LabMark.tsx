...
const LabMark = () => {
  ...
  const loadMarks = async () => {
    try {
      ...
      if (studentId) {
        const saved = localStorage.getItem(`lab_marks_${studentId}`);
        if (saved) {
      ...
      console.error('Error loading Lab marks:', error);
    }
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <h1 className="page-header">LAB Mark</h1>
      
      {/* Admin Editor */}
      {isAdmin() && <AdminMarksEditor type="lab" title="Manage Lab Marks" maxMarks={100} />}
              ...
              <table className="data-table">
                ...
                <tbody>
                  {subject.marks.map((mark: { subjectName: string; subjectCode: string; score: number; maxMarks: number; remarks: string; date: string }, index: number) => {
                    ...
                    return (
                      <tr key={index}>
                        <td className="font-medium">Exp {mark.testNumber}</td>
      ...
      {marks.length === 0 && (
        <Card className="border-none shadow-card">
          <CardContent className="p-12 text-center">
            <FlaskConical className="w-16 h-16 text-muted-foreground/20 mx-auto mb-4" />
            <p className="text-muted-foreground">No lab marks available yet.</p>
          </CardContent>
        </Card>
      )}
    </div>
